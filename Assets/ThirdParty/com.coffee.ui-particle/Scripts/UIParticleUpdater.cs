...
namespace Coffee.UIExtensions
{
    internal static class UIParticleUpdater
    {
		...
		public static void DumpInfo(StringBuilder result)
		{
			var psTotal = s_ActiveParticles.Sum(x => x.particles.Count);
			
			result.AppendLine($"UIParticles total ui/ps/particles#={s_ActiveParticles.Count}/{psTotal}/{particleCount}");

			foreach ((var name, var count) in s_ActiveParticles.Select(x => (x.name, count: x.particles.Sum(z => z.particleCount)))
        ...
        private static void BakeMesh(UIParticle particle)
        {
            ...
            for (var i = 0; i < particle.particles.Count; i++)
            {
				...
				particleCount += currentPs.particleCount;
